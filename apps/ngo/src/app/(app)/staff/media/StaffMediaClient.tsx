...
// Deliberately identical body to HodMediaClient — same media table, same
// upload flow, same department filter. Only real difference: usePageTitle
// ('Media Library') to match this page's own NAVMAP label (shared with
// HOD's and leadership's — all three workspaces have a page by this name).
...
export function StaffMediaClient({
  orgId,
  department,
  employeeCode,
  employeeName,
  ...
  async function handleUpload(e: React.FormEvent<HTMLFormElement>) {
    ...
    // this exact prefix via storage.foldername(name)[1].
    ...
    if (insertErr) {
      // Same orphan-file gap already noted on leadership/media and
      // hod/media: the file is in storage but has no row, not cleaned up
      // automatically.
  ...
  return (
    <div className="space-y-4">
      ...
      <Card title="Department media" subtitle={`${items.length} files in ${department}`}>
        {items.length === 0 ? (
          <p className="text-[0.72rem] text-muted py-2">Nothing uploaded for this department yet.</p>
        ) : (
